...
export default function ChatBot() {
  ...
  return (
    <div className="fixed inset-x-4 bottom-4 sm:inset-x-auto sm:right-6 sm:bottom-6 z-1000 flex flex-col items-stretch sm:items-end w-auto">
      {/* ---------- MODAL CHAT ---------- */}
      {openChatbot && (
        <div
          className="flex flex-col w-full max-w-full sm:max-w-[400px] bg-white rounded-2xl shadow-2xl overflow-hidden max-h-[70vh] sm:max-h-[80vh]"
          data-aos="zoom-in"
        ...
        >
          ...
          {/* Body */}
          <div className="flex-1 flex flex-col gap-2 min-h-[120px] max-h-40 sm:max-h-56 md:max-h-[50vh] overflow-y-auto px-3 sm:px-4 py-3 sm:py-4">
            {history.map((m) => (
              <div
                key={m.id}
                className={`inline-block px-3 py-2 rounded-3xl text-white max-w-[80%] wrap-break-word
              ${
                m.sender === "bot"
          ...
          <div className="drop-shadow-2xl shadow-2xl">
            <div className="flex flex-wrap justify-end gap-2 sm:gap-3 bg-[#FFF3E6] p-3 sm:p-4 max-h-[40vh] overflow-y-auto">
              {actions.map((a, idx) =>
                a.type === "button" ? (
                  <button
                    key={idx}
                    onClick={() => handleBtn(a.label, a.next)}
                    className="inline-block px-3 py-2 rounded-3xl text-xs sm:text-sm font-medium text-white bg-[#FCA13B] whitespace-nowrap"
                  >
                    {a.label}
                  </button>
                ) : (
      ...
      <button
        ...
        className="mt-3 bg-[#FCA13B] hover:bg-[#FCA13B]/90 text-white p-3 sm:p-4 rounded-full cursor-pointer shadow-lg focus:outline-none self-end"
